...
3. Создайте файл `hello_world.cpp` в локальной копии репозитория (который должен был появиться на шаге 2). Реализуйте программу **Hello world** на языке C++ используя плохой стиль кода. Например, после заголовочных файлов вставьте строку `using namespace std;`.<details>
...
7. **commit**, **push**.
   ```
git commit -am "Добавлены коментарии к hello_world.cpp"
   ```
      <details>
   <summary>output</summary>
>[patch1 6fee835] Добавлены коментарии к hello_world.cpp
...
branch 'patch1' set up to track 'lab02/patch1'.
   ...
   10. Локально выполните **pull**.*<details>
   <summary>output</summary>
``` 
git switch main
...
Date:   Fri Mar 6 21:19:58 2026 +0300
    ...
    Добавлены коментарии к hello_world.cpp
...
2. Измените _code style_ с помощью утилиты [**clang-format**](http://clang.llvm.org/docs/ClangFormat.html). Например, используя опцию `-style=Mozilla`.
   ...
   ```
<details>
<summary> output </summary>Следующий пакет устанавливался автоматически и больше не требуется:
...
4. В ветке **master** в удаленном репозитории измените комментарии, например, расставьте знаки препинания, переведите комментарии на другой язык.<details>
<summary> hello_world. cpp измененный </summary>
